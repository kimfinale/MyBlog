...
image: "Mathematica_logo.svg.png"
editor_options: 
...
A [recent article](https://www.mdpi.com/2227-7390/12/1/27) published in Mathematics 
discusses an approach to calculating $\mathcal{R}_0$. Since I have previously written a [post](https://www.jonghoonk.com/posts/R0-sympy/) about calculating $\mathcal{R}_0$ using Sympy, I wanted explore a new algorithmic approach proposed by the article. 

The article claims that $\mathcal{R}_0$ is a not function of the original set of ordinary differential equation because $(F, V)$ gradient decomposition may not be unique for a given set of ODE. As a reminder, [Diekmann *et al.*](https://link.springer.com/article/10.1007/BF00178324) showed that $\mathcal{R}_0$ is the spectral radius, or Perron–Frobenius eigenvalue, of the next generation operator. 
...
if $\beta$ and $\gamma$ are replaced with $b$ and $g$, respectively.
```{python}
from sympy import *
b, k, g, = symbols('b k g')
F = Matrix([[0, b],[0, 0]])
