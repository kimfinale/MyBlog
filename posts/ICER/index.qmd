...
If someone dies at time 0 and his or her life expectancy at the age is $L$, then
the YLL would be $L$. However, better representation would account for dicounting
rate over the years.  The following equation representing exponential decay can
be used to compute YLL while accounting for discounting rate, $r$.
...
for (i in 1:L){
  first_yr_dt <- (1/r)*(1-exp(-r)) # first year discounted < 1
  # Take the 1 over its value to compute the past
  if (i < refyr) { first_yr_dt <- 1/first_yr_dt }
