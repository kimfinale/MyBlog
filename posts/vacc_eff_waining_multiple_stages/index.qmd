...
First, let's construct the classic SEIR model for comparison. This model is 
equivalent to a variant with variable compartments for the $I$ stage when considering only a single compartment.
...
The model with variable compartments for the $I$ stage. A part of the model
is written using strings using `paste`, which are later evaluated using `eval(str2lang("some string"))`.
```{r}
