...
In this post, we explore how to parameterize the disease transmission model (e.g., $SEIR$ model) when one wished to incorporate vaccination campaign and waning of vaccine-induced immunity. One way to model this is to assume partially immunity among individuals in the vaccinated state ($V$ compartment) and the individuals in the $V$ compartment gradually move to the susceptible state ($S$ compartment). This is a simple approach that may mimic the setting in which vaccine efficacy trial was conducted. I will use typhoid vaccine as an example. I followed the method that was presented in the [prior post]().

### Vaccine efficacy data

Vaccine efficacy data come from a clinical trial conducted in Malawian children over the period of Typhoid conjugate vaccine, Vi tetanus toxoid, were given to the children
...
Solutions for the steady states of the $SEIR$ model. These are used to set the initial conditions as we assume that the disease transmission is in a steady state
...
Define sum of squared difference (ssq) to evaluate the difference between the model and the observation
...
plot(times/365, ve_obs, col="firebrick", 
     ylim=c(0.5,1),  
     xlab="year since vaccination", ylab="direct vaccine effectiveness",
     type="p", pch=0)

lines(times/365, measure_vacc_eff(p=c(0.88,1/(20*365)), params=params, times=times), col="black")
```

```{r}
params[["nstg"]] <- 1
fit <- nlminb(c(0.9, 1/(100*365)), objective=ssq, 
              lower=c(0.1,1/(100000*365)),
              upper=c(0.99,0.99))
fit$par[1]
1/fit$par[2]/365
ve_obs
round(measure_vacc_eff(p=fit$par, params=params, times=times), digits=3)
```
...
plot(times/365, ve_obs, col="firebrick", 
     ...
     type="p", pch=0)
lines(times/365, measure_vacc_eff(p=fit$par, params=params, times=times), col="black")
legend("topright", 
  legend=c("Data", "Model"), 
