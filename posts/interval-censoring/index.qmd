...
{A}_i)$$
, where $f$ is a probability density function for the serial interval, which we assume follows a Gamma distribution.
...
$$\mathcal{L} = \prod_{i=1}^{n} \int_{t^{A}_{Li}}^{t^{A}_{Ri}} g(\tau) f(t^{B}_i-\tau) ~\text{d}\tau$$
