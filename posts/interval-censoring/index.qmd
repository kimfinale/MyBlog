...
### Vanialla maximum likelihood estimation

Suppose dates of onsets of infectors, $t^{A}$, and infectees $t^{B}$, are given specific dates.  Then, serial interval may be estimated as follows:  
...
Now suppose that the dates of onset of the infectors are given only as intervals. In this case, the above likelihood function may be modified as follows:

$$\mathcal{L} = \prod_{i=1}^{n} \int_{t^{A}_{iL}}^{t^{A}_{iR}} g(\tau) f(t^{B}_i-\tau) ~\text{d}\tau$$
, where $A_L, A_R, t$ present the times for lower end and upper end of the interval for the time of onset of the infector, and the onset time of the infectee, respectively. $g(x)$ represents the density function to represent the interval and we assume it follows a uniform distribution. 
```{r uniform_interval}
set.seed(42)
