...
```{r}
fit1 = optim(par=log(0.3), fn=negloglik, y=dat2, method="Brent", lower=log(1e-6), upper=log(0.9))
fit2 = optim(par=c(log(0.3), logit(0.2)), fn=negloglik, y=dat2, method="Nelder-Mead")
...
aic(k=1, L=exp(-fit1$value)) 
aic(k=2, L=exp(-fit2$value)) 
aicc(k=1, L=exp(-fit1$value), n=length(dat2))
aicc(k=2, L=exp(-fit2$value), n=length(dat2))
```
