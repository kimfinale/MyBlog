...
I can seem to use the third equation to reproduce the answer to the example the authors provided.  On page 603, the authors gave the example of Influenza A where the generation interval has a mean of 2.85 days with the standard deviation of 0.93 days. The epidemic growth rate $r = 0.2$. "The $R=1.57$ for the SIR epidemic
model, a value of $R=1.65$ for the SEIR
epidemic model and a value of $R=1.66$ the more complicated epidemic model with one latent stage
and two infectious stages (equation (3.3), with $x=1, y=2$)" 
...
For the model with $x=1, y=2$, the $R$ is not the same as provided. 
$R$ can vary depending on how we set $b_1$ and $b_2$, but it is smaller than one.
```{r}
