...
date: "2023-12-19"
categories: [universal differential equation, python, SIR]
...
Follwoing python codes were copied from the [GitHub site](https://github.com/ai-research-disi/ode-discovery-with-ude/blob/main/models/sir/custom_models.py)
...
from custom_models import UnrolledSIR, PairedSIR, EndtoEndIntervetionMap, SIR, build_model
...
def test_unrolled(y0, n_days, beta_range, gamma, steps, epochs, callbacks=[], noise=None, n_iter=100): 
    ...
    # df.to_csv(filename)
    df.to_csv('unrolled_10000.csv', mode='a', header=False)
```
...
def test_supervised(y0_base,
    ...
    df = pd.DataFrame.from_dict(results)
    # df.to_csv(filename)
    df.to_csv('supervised_10000.csv', mode='a', header=False)
```
...
def test_observable(y0_base,
    ...
    df.to_csv('observable_2.csv', mode='a', header=False)
```
...
def test_sampling(y0_base,
    ...
    df = pd.DataFrame.from_dict(results)
    # df.to_csv(filename)
    df.to_csv('sampling_2.csv', mode='a', header=False)
```
