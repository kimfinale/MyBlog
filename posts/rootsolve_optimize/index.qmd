...
We can remove $\dot{R}$ since $S+I+R=1$. From the equation, we can have the following relationship.
 
 $$\frac{dS}{dI}= -1 + \frac{\gamma}{\beta S}$$
 Solving this equation gives the following: $$ I(t) = -S(t) + \frac{\gamma}{\beta} \text{ln} S(t) + C$$
...
$$S(\infty) = 1 − \text{exp}\left[-R_0\left(1-S(\infty)\right)\right]$$
Using the $R(\infty)=1-S(\infty)$, we can get the following equation for the final size of an epidemic, $R(\infty)$:


$$R(\infty) = 1 − \text{exp}\left[-R_0R(\infty)\right]$$
Let's use the above relationship to compute the final epidemic size nuerically
```{r}
