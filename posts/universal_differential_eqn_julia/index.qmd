...
date: "2024-01-02"
categories: [universal differential equation, julia, Lotka-Volterra, predator-prey model]
...
I previously took an [approach using Python]() but at least for the topic of 
universal differential equation, Julia seems to be an easier option. 
The following Julia codes were adopted from the blog post on [DiffEqFlux.jl](https://julialang.org/blog/2019/01/fluxdiffeq/). To use Julia
in Quarto, I installed IJulia and Revise packages in my Julia environment, as 
described [here](https://quarto.org/docs/computations/julia.html). But then, 
I had to eventually install JuliaCall package in my R environment. I was able to
render the qmd document anyhow by just using `{julia}` code block. However, 
I don't know whether the installing IJulia and Revise packages in Julia 
