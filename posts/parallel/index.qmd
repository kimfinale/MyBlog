...
I find that using `parallel`, `doParallel` and `foreach` provides the easiest approach.
The `doParallel` [vignette](https://cran.r-project.org/web/packages/doParallel/vignettes/gettingstartedParallel.pdf) provides a great overview.  
...
x <- 2
res <- foreach(i=1:4, .inorder=F, .combine=c) %dopar% { c(i, x^2) }
...
res <- 
  foreach(i=1:5, .packages=c("data.table"), .inorder=F, .combine='cbind') %dopar% {
...
`foreach` sessions are not reproducible
...
We may want to examine the status of parallel workers.
